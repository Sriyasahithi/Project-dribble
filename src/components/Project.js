...
const Project = ({images}) => {
  const [likes, setLikes] = useState(parseInt(images.likes, 10))
  ...
  return (
    ...
    <div className="flex justify-between p-2">
        <div className="flex items-center">
            <h3 className="font-bold text-black text-[15px]">{images.name}</h3>
            <h3 className=" bg-gray-300 text-white p-[2px] text-xs font-bold rounded-md ml-2">{images.team}</h3>
        </div>
        <div className="flex space-x-2">
            <div className="flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5" onClick={handleClick}>
